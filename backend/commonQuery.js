        ...
        $project: {
          ...
          full_name: {
            $concat:[
              ...
              "$user_info.firstName",
              {
                 $cond: { if: { $ne: [ "$user_info.middleName", null ] }, then: " ", else: "" }
               }
              ,
...
commonQuery.fetch_all_motivation = function fetch_all_motivation(
  ...
  return new Promise(function (resolve, reject) {
    ...
    let postQuery = model.aggregate([
      { $match: { "hospital_id" : mongoose.Types.ObjectId(hospital_id),"isDeleted" : false} },
      {
        $lookup:
          {
              from: "users",
              let: { patient_id: "$patient_ids" },
                pipeline: [
                         ...
                         {$project:{fullName:{$concat:["$firstName"," ","$middleName"," ","$lastName"]}}}
...
commonQuery.findPatientQuestion = function findPatientQuestion(
  ...
  return new Promise(function (resolve, reject) {
    model
      .aggregate([
        { $match: { isDeleted:false ,assessment_id: mongoose.Types.ObjectId(assessment_id) } },
        {
          $lookup: {
            from: "patientassessments",
...
commonQuery.responseQuery = function responseQuery(model, id, dataToAdd) {
  return new Promise(function (resolve, reject) {
    model
      .findByIdAndUpdate(
        id,
        { $push: { responses_id: dataToAdd } },
        { safe: true, upsert: true }
      )
      .exec(function (err, userData) {
        if (err) {
